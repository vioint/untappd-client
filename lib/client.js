...
class UntappdClient {
	...
	constructor(opts) {
		...
		this.validateParam = function (param, key) {
			if (param === null) {
	...
	*/
	breweryInfo(breweryId, opts, callback) {
		this.validateParam(breweryId, 'BREWERY_ID');
	...
	*/
	...
	checkin(bid, timezone, gmt_offset, opts, callback) {
		...
		this.validateParam(bid, 'bid');
		this.validateParam(timezone, 'timezone');
		this.validateParam(gmt_offset, 'gmt_offset');
		opts = Object.assign({
			'bid': bid,
			'timezone': timezone,
